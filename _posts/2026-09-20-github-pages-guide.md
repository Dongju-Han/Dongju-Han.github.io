...
- `README.md`: 현재는 사이트 첫 화면에 보여 주는 기본 내용이다.
...
GitHub Pages는 보통 `index.html`, `index.md`, `README.md` 중 하나를 사이트의 첫 파일로 사용한다. 이 레포에는 아직 `index.md`가 없으므로 현재 홈 화면은 `README.md`를 바탕으로 만들어져 있다.
...
한국에서 작성한 글은 `2026-09-20 21:00:00 +0900`처럼 시간대까지 적으면 날짜가 다른 날로 바뀌는 문제를 줄일 수 있다. 나중에 글을 수정해도 원래 게시 날짜를 유지하고 싶다면 `date` 값을 바꾸지 않으면 된다.
...
## 8. 홈 화면에 글 목록이 보이지 않는 이유

새 글을 만들었다고 해서 현재 홈 화면에 글 목록이 자동으로 생기는 것은 아니다. 지금 홈은 `README.md`를 보여 주는 기본 화면이고, 글 목록을 출력하는 Liquid 코드가 아직 없기 때문이다.

글 자체는 기본 URL에서 열리지만, 홈에서 최근 글을 클릭할 수 있게 하려면 나중에 `index.md`를 만들면 된다. `index.md`를 만들면 `README.md` 대신 이 파일이 홈 화면의 내용이 된다.
...
이 코드는 `site.posts`에 들어 있는 글을 순서대로 돌면서 제목, 링크, 날짜를 출력한다. 처음 글을 올리는 단계에서는 `index.md`까지 만들지 않고 글의 직접 URL부터 확인해도 된다.
...
`README.md`는 홈 화면의 본문일 뿐이고, `_posts`의 글들을 자동으로 나열하는 코드를 포함하고 있지 않다. 홈에 목록을 만들려면 앞 장의 `index.md`와 `site.posts` 반복문이 필요하다.
...
글이 몇 개 쌓인 다음에는 `index.md`로 최근 글 목록을 만들고, 그다음에 테마와 CSS를 손보는 것이 좋다. 글쓰기와 디자인을 동시에 시작하면 어디에서 문제가 생겼는지 찾기 어려우므로, 먼저 글 하나를 끝까지 공개하는 경험을 만드는 것이 우선이다.
